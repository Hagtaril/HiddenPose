'''
pp_net
    output: (batch_size = 1, t = 128, c = 1, h = 64, w = 64 )

-> input : (1, channels = 3, h = 256, w = 256)
...
class TokenPose_L(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.transformer = TokenPose_L_base(feature_size=[cfg.MODEL.IMAGE_SIZE[1] // 4, cfg.MODEL.IMAGE_SIZE[0] // 4],
                                            ...
                                            hidden_heatmap_dim=cfg.MODEL.HIDDEN_HEATMAP_DIM,  # heatmap_dim = 384
                                            heatmap_dim=cfg.MODEL.HEATMAP_SIZE[1] * cfg.MODEL.HEATMAP_SIZE[0],
                                            # heatmap_size = [64, 64], heatmap_dim = 64 * 64
...
class TokenPose_L_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False,
                 ...
                 emb_dropout=0., pos_embedding_type="learnable"):
        super().__init__()
        ...
        num_patches = (feature_size[0] // (patch_size[0])) * (
                    feature_size[1] // (patch_size[1]))  # feature_size = [64, 64]; patch_size = [4, 4]
...
if __name__ == '__main__':
    print(tokenpose_cfg.MODEL)
    model = TokenPose_L(tokenpose_cfg)
