...
import numpy as np
import cv2
from utils.helper import definePsf, resamplingOperator
...
def lct(meas_hxwxt, wall_size, crop, bin_len):
    ...
    cv2.imshow("front", front_view_HxW / np.max(front_view_HxW))
    # cv2.imshow('gt', imgt)
    cv2.waitKey()
    ...
    left_view = np.max(volumn_ZxYxX, axis = 1)
    cv2.imshow("left", left_view / np.max(left_view))
    cv2.waitKey()

    top_view = np.max(volumn_ZxYxX, axis = 2)
...
if __name__ == '__main__':
    ...
    lct(rect_data_hxwxt, wall_size=1, crop=crop, bin_len=bin_len)
